...
const handler = async (req: NextApiRequest, res: NextApiResponse) => {
  const hexRe = /^([A-Fa-f0-9]{6}|[A-Fa-f0-9]{3}|[A-Fa-f0-9]{8}|[A-Fa-f0-9]{4})$/
  const numRe = /^\d+$/
  let flag = false
  Object.entries(req.query).forEach(([key, value]) => {
    ...
    if (['hair', 'eye', 'bg', 'stroke'].includes(key) && (!hexRe.test(value) && !['transparent', 'currentColor'].includes(value))) {
      flag = true
      return
    }
  ...
  let { hair, eye, bg, stroke, strokeWidth, size, format } = req.query as { [key: string]: string }
